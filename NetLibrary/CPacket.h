...
public:
	...
	// - 사용 끝난 후 ReleasePacketView()호출 필요
	//----------------------------------------------------------
	static CPacketView* AllocPacketView(CPacket* refPacket, ULONG dataSize);
	...
	//----------------------------------------------------------
	static void ReleasePacketView(CPacketView* packetViewer);
	...
	// 연산자 오버로딩
	//--------------------------------------------------------------
	inline CPacketView& operator >> (unsigned char& byValue)
	{
		byValue = *(unsigned char*)readPtr;
		readPtr += sizeof(unsigned char);
		useSize -= sizeof(unsigned char);
	...
	inline CPacketView& operator >> (char& chValue)
	{
		chValue = *(char*)readPtr;
		readPtr += sizeof(char);
		useSize -= sizeof(char);
	...
	inline CPacketView& operator >> (unsigned short& wValue)
	{
		wValue = *(unsigned short*)readPtr;
		readPtr += sizeof(unsigned short);
		useSize -= sizeof(unsigned short);
	...
	inline CPacketView& operator >> (short& shValue)
	{
		shValue = *(short*)readPtr;
		readPtr += sizeof(short);
		useSize -= sizeof(short);
	...
	inline CPacketView& operator >> (unsigned int& iValue)
	{
		iValue = *(unsigned int*)readPtr;
		readPtr += sizeof(unsigned int);
		useSize -= sizeof(unsigned int);
	...
	inline CPacketView& operator >> (int& iValue)
	{
		iValue = *(int*)readPtr;
		readPtr += sizeof(int);
		useSize -= sizeof(int);
	...
	inline CPacketView& operator >> (unsigned long& dwValue)
	{
		dwValue = *(unsigned long*)readPtr;
		readPtr += sizeof(unsigned long);
		useSize -= sizeof(unsigned long);
	...
	inline CPacketView& operator >> (long& dwValue)
	{
		dwValue = *(long*)readPtr;
		readPtr += sizeof(long);
		useSize -= sizeof(long);
	...
	inline CPacketView& operator >> (float& fValue)
	{
		fValue = *(float*)readPtr;
		readPtr += sizeof(float);
		useSize -= sizeof(float);
	...
	inline CPacketView& operator >> (double& dValue)
	{
		dValue = *(double*)readPtr;
		readPtr += sizeof(double);
		useSize -= sizeof(double);
	...
	inline CPacketView& operator >> (unsigned __int64& iValue)
	{
		iValue = *(unsigned __int64*)readPtr;
		readPtr += sizeof(unsigned __int64);
		useSize -= sizeof(unsigned __int64);
	...
	inline CPacketView& operator >> (__int64& iValue)
	{
		iValue = *(__int64*)readPtr;
		readPtr += sizeof(__int64);
		useSize -= sizeof(__int64);
	...
	inline char* GetReadPtr() { return readPtr; };
	inline static ULONG GetPacketViewAllocCount() { return packetViewPool.GetAllocCnt(); };
	inline static ULONG GetPacketViewChunkPoolCount() { return packetViewPool.GetChunkPoolCnt(); };
	inline static ULONG GetPacketViewEmptyPoolCount() { return packetViewPool.GetEmptyPoolCnt(); };
...
private:
	...
	static CObjectPool_TLS<CPacketView, true> packetViewPool;
};